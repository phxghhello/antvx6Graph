...
<script>
// 工具栏
import { onPaste, onCopy, selectAll, deleteCells } from "../../common/trigger";
    ...
    const methods = {
      ...
      handleTrigger(name) {
        switch (name) {
          ...
          case "undo":
          case "redo":
            if (data.tools[name].can) graph.value[name]();
            break;
          case "zoomin":
      ...
      Channel.eventListener(CustomEventTypeEnum.FREEZE_GRAPH, (bool) => {
        ...
        for (const key in data.tools) {
          if (Object.hasOwnProperty.call(data.tools, key)) {
            ...
            if (["undo", "redo", "copy", "paste", "select_all"].includes(tool.svg)) {
              tool.freeze = bool;
            }
          }
        }
